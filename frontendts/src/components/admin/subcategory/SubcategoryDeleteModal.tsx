...
import { useDeleteSubcategoryByIdMutation } from '@/services/api/subcategoryApi';
...
const SubcategoryDeleteModal = ({
  ...
  const [deleteSubcategory, { isLoading }] = useDeleteSubcategoryByIdMutation();

  const handleDelete = async () => {
    if (!subcategory) return;
    try {
      await deleteSubcategory(subcategory.subcategory_id).unwrap();
      showToast({ message: 'Xóa danh mục con thành công!', type: 'success' });
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-yellow-50 px-6 py-4 border-b border-yellow-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-yellow-900">Cảnh báo xóa danh mục con</h2>
                <p className="text-sm text-yellow-700">
                  Hành động này sẽ xóa vĩnh viễn danh mục con và không thể hoàn tác.
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-6">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 mb-2">
                    Mã ID: <span className="font-medium">{subcategory.subcategory_id}</span>
                  </p>
                  <p className="text-xs text-gray-500">
                    Trạng thái:{' '}
                    <span
                      className={
                        subcategory.status === 'active' ? 'text-green-600' : 'text-red-600'
                      }
                    >
                      {subcategory.status === 'active' ? 'Hoạt động' : 'Vô hiệu hóa'}
                    </span>
                  </p>
                </div>
              </div>
            ...
            <div className="mb-6">
              <p className="text-gray-700 mb-4 text-base leading-relaxed">
                ...
                <span className="font-bold text-gray-900">{subcategory.subcategory_name_vn}</span>{' '}
                không?
              </p>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
              ...
              <ul className="space-y-2">
                <li className="flex items-start gap-2 text-gray-700">
                  ...
                  <span>Xóa vĩnh viễn danh mục con này khỏi hệ thống</span>
                </li>
                <li className="flex items-start gap-2 text-gray-700">
                  <List className="w-4 h-4 text-gray-500 mt-0.5 shrink-0" />
                  <span>Xóa tất cả các sản phẩm, loại sản phẩm liên quan</span>
                </li>
                <li className="flex items-start gap-2 text-gray-700">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              <p className="text-sm text-blue-800 flex items-start gap-2">
                ...
                <span>
                  <strong>Lưu ý:</strong> Nếu danh mục con này đang được sử dụng trong sản phẩm hoặc
                  các bảng liên quan, bạn sẽ không thể xóa nó. Hệ thống sẽ thông báo lỗi khi thực
                  hiện xóa.
                </span>
              </p>
            </div>
